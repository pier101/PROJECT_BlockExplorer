...
function connectToPeers(newPeers) {
    ...
    newPeers.forEach(peer=>{
        ...
        ws.on("open",()=> {
            console.log("open 3003")
            initConnection(ws)
        })
        ws.on("error",()=>console.log("connection Failed! 3003"))
    })
}   
...
function initMessageHandler(ws){
    ws.on("message",(data)=>{
        ...
        // console.log('받은 메세지 3003 : ' + JSON.stringify(message));
        switch (message.type) {
            case MessageType.QUERY_LATEST:
                console.log("메세지 0  3003")
                write(ws,responseLatestMsg());
                break;
            case MessageType.QUERY_ALL:
                console.log("메세지 1  3003")
                write(ws,responseAllChainMsg());
                break;
            case MessageType.RESPONSE_BLOCKCHAIN:
                console.log("메세지 2  3003")
